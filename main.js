...
// 連打1回あたりの玉の成長量
const TAP_GROWTH = 0.06;

// 時間経過による成長：20秒で連打35回分くらいの成長量になるようにする
const TIME_GROWTH_PER_SECOND = (TAP_GROWTH * 35) / 20;
...
function tapPower() {
    ...
    ballScale += TAP_GROWTH;

    spiritBall.scale.set(ballScale, ballScale, ballScale);

    ballMaterial.emissiveIntensity += 0.06;
...
function updateStar() {
    ...
    for (const starMesh of starParticles) {
        ...
        starMesh.position.add(starMesh.userData.velocity);
        starMesh.userData.velocity.multiplyScalar(0.97);

        starMesh.rotation.x += starMesh.userData.spin.x;
        starMesh.rotation.y += starMesh.userData.spin.y;
        starMesh.rotation.z += starMesh.userData.spin.z;

        starMesh.scale.multiplyScalar(1.01);
        starMesh.material.opacity -= 0.012;
    }
...
function updateSpiritBall(deltaSeconds) {
    ...
    // 時間経過による自動成長（20秒で連打50回分くらい）
    if (gameStarted && !gameFinished) {

        ballScale += TIME_GROWTH_PER_SECOND * deltaSeconds;
        spiritBall.scale.set(ballScale, ballScale, ballScale);
    }

    // 色は毎フレーム更新し、閾値を超えた瞬間に即座に切り替わるようにする
    updateBallColor();
}
